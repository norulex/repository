...
def my_seq2seq_with_atten(anotherInput, encoder_inputs, decoder_inputs, cell,
                                num_encoder_symbols, num_decoder_symbols,
                                num_heads=1, output_projection=None,
                                feed_previous=False, dtype=dtypes.float32,
                                scope=None, initial_state_attention=False):
  ...
  with variable_scope.variable_scope(scope or "embedding_attention_seq2seq"):
    ...
    # alphabeet Encoder
    cell_forward = rnn_cell.EmbeddingWrapper(rnn_cell.GRUCell(16) , 38)
    cell_backWord = rnn_cell.EmbeddingWrapper(rnn_cell.GRUCell(16) , 38)

    initFw = tf.zeros([8,16],"float")
    initBw = tf.zeros([8,16],"float")

    alphabetEnc = rnn.bidirectional_rnn(cell_forward,cell_backWord, anotherInput , initFw , initBw)


    # Encoder.
    encoder_cell = rnn_cell.MyEmbeddingWrapper(cell, num_encoder_symbols)

    encoder_outputs, encoder_state = rnn.my_rnn(
        alphabetEnc , encoder_cell, encoder_inputs, dtype=dtype)
...
def model_with_buckets(anotherInput, encoder_inputs, decoder_inputs, targets, weights,
  ...
  all_inputs = anotherInput + encoder_inputs + decoder_inputs + targets + weights
  losses = []
  outputs = []
  with ops.op_scope(all_inputs, name, "model_with_buckets"):
    for j, bucket in enumerate(buckets):
      with variable_scope.variable_scope(variable_scope.get_variable_scope(),
        ...
        bucket_outputs, _ = seq2seq(anotherInput[:bucket[0]],
